...
try {
  if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
    ...
    kv = vercelKv.kv;
    useKV = true;
    console.log('[Serverless KV] Active cloud store initialized.');
  }
} catch (e) {
  console.warn('[Warning] @vercel/kv load failed inside Serverless function.');
}
...
async function callRESTWithRetry(modelName, apiKey, payload, maxRetries = 5) {
  // Tighten intervals for Serverless 10s timeout limit
  const retryIntervals = [1000, 1500, 2000, 2000, 2000];
...
// Export Serverless Handler for Vercel Function
...
- 절대로 설명이나 마크다운 백틱 코드블록(\`\`\`json 등)을 두르지 말고 오로지 파싱 가능한 순수 JSON 텍스트 하나만 리턴해줘. JSON 규격:
{
  ...
  "hashtags": "[해시태그 10개]"
      ...
      let textResult = await callRESTWithRetry('gemini-3.5-flash', apiKey, textPayload, 5);
      ...
      if (parsed && contentType === 'blog') {
        ...
        for (let i = 0; i < prompts.length; i++) {
          ...
          try {
            ...
            console.error('Image API error:', imageErr.message);
            images.push({
              url: 'error',
      ...
      console.error('Gemini call error:', apiErr.message);
    ...
    await sendResponse(generatedResult, counts);
    ...
    console.error('Critical processing breakdown:', err.message);
    res.status(500).json({ success: false, error: 'Internal processing breakdown.' });
  }
};
