...
[스레드 본문 조건 - 필수 준수]
- 분량: 전체 3~4줄, 총 40~60자 내외로 아주 짧게 쓸 것. 읽는 사람이 5초 안에 전부 다 읽을 수 있는 분량이어야 함 (절대 그 이상 길어지지 말 것).
- 말투: 친구한테 말하듯 편한 반말체 (~야 / ~해 / ~거든 / ~더라)를 사용할 것. 존댓말 절대 금지.
- 구성 (반드시 이 3단 구조를 지킬 것):
  1) 훅(Hook) 1줄: 첫 줄에서 바로 시선을 확 붙잡는 강렬한 한마디 (반전형, 공감형, 단언형 중 하나). 물음표로 끝나는 질문형 문장은 절대 금지 (CTA에서만 질문을 사용할 것).
  2) 핵심 메시지 1~2줄: 설명하지 말고 임팩트로 승부. 이 업계의 뻔한 방식과 다른 이 사업자만의 의외의 태도/기준을 아주 짧게 던질 것 (예: "저는 손님 늘리는 것보다 ○○가 더 중요해요" 같은 대비 한 줄)
  3) CTA 1줄: 마지막 줄은 반드시 팔로우/저장/댓글/DM 중 하나를 유도하는 짧은 질문형 문장으로 마무리할 것 (예: "너는 어때?", "저장해두고 볼래?" 처럼 물음표로 끝나는 문장. 명령형 문구 금지)
- 금지 사항: 절대로 AI 냄새가 나거나, 뻔한 광고체, 상투적인 어조를 쓰지 말 것. (특히 '오늘은~알아보겠습니다', '결론부터', '첫째둘째셋째', '따라서', '즉' 같은 단어는 사용 절대 금지. 존댓말 어미도 금지)
- 이모지/이모티콘은 절대 사용하지 말 것 (텍스트로만 작성).
- 가독성: 3~4줄 각각을 짧게 끊어 줄바꿈(개행)할 것. 긴 문장/설명조 문장 금지.
...
- 절대로 설명이나 마크다운 백틱 코드블록(\`\`\`json 등)을 두르지 말고 오로지 파싱 가능한 순수 JSON 텍스트 하나만 리턴해줘. JSON 규격:
{
  ...
  "selected_topic": "[선정된 주제 1개]",
  "title": "[스레드 메인 주제 키워드]",
  "body": "[3~4줄, 총 40~60자 내외의 초단문 반말체 스레드 본문 (1줄 훅 -> 1~2줄 핵심 메시지 -> 물음표로 끝나는 CTA 1줄, 이모지 없이 줄바꿈만 적용)]",
  "hashtags": "[해시태그 10개]"
}`;
}
